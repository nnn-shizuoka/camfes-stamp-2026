  ...
  return (
    ...
    >
      <div className="relative aspect-square w-full overflow-hidden">
        <p className="absolute left-1 top-1 text-left text-[0.62rem] font-black uppercase leading-none tracking-[0.12em] text-[#4a3319]">
          No.{stamp.id}
        </p>
        {collected && !imageFailed ? (
          <div className="absolute inset-0 flex items-center justify-center">
              <img
                ...
                alt={`${stamp.name} スタンプ`}
                className="h-[80%] w-auto translate-y-1 select-none opacity-100"
                draggable={false}
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="h-[70%] w-[70%] translate-y-1 rounded-full border-2 border-[#8a6d3b] bg-transparent" />
          </div>
        )}
      </div>
    </div>
